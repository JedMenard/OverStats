...
async def testing(loop):
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
        ...
        #data[PC] = await client.get_profile("Fuzzy#1393", session=session, platform=PC)
        #print('Testing......[get_stats]')
        #data[PC] = await client.get_stats("Fuzzy#1393", session=session, platform=PC)
        print('Testing......[get_hero_stats]')
        data[PC] = await client.get_hero_stats("Fuzzy#1393", session=session, platform=PC)
...
if __name__ == "__main__":
    ...
    print("Done!")
    for key in data[PC]['us']['stats']['competitive']['mercy']['general_stats'].keys():
        print(key)
